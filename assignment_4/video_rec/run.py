...
from test import test, get_test_report, get_confusion_matrix, get_auc_score, plot_confusion_matrix
...
def args_parser():
    ...
    # Directory arguments — separate train/val/test
    parser.add_argument('-td', '--train_dir', help='Directory containing training frames')
    parser.add_argument('-vd', '--val_dir', help='Directory containing validation frames')
    parser.add_argument('-tsd', '--test_dir', help='Directory containing test frames')
    ...
    parser.add_argument('-op', '--output_path', type=str, default='/users/jmatthia/deep_learning/code/assignment_4/', help='Directory to save output files')
...
def main(args):
    ...
    if mode == 'train':
        ...
        tr_dataset_raw, label_dict = load_dataset(args.train_dir)
        val_dataset_raw, _ = load_dataset(args.val_dir)
        ...
        print(f"Train samples: {len(tr_dataset_raw)} | Val samples: {len(val_dataset_raw)}")
        print(f"Device: {device}")

        # Create PyTorch Datasets and DataLoaders for train and validation
        tr_split = list(tr_dataset_raw.items())
        val_split = list(val_dataset_raw.items())
        ...
        # Load test dataset directly from test directory
        ts_dataset_raw, label_dict = load_dataset(args.test_dir)
        ts_split = list(ts_dataset_raw.items())

        print(f"Test samples: {len(ts_split)}")
        ...
        model.to(device)
        targets, outputs, probs, accuracy = test(model, dataloaders['test'], device)

        print("----- Metrics from test set -----")
        print("outputs:", outputs, "probs:", probs, "accuracy:", accuracy)

        print(f'Test accuracy: {100 * accuracy:.4f}%')

        all_cats = sorted(label_dict.keys())

        print(f'Macro ROC AUC: {get_auc_score(targets, probs):.4f}')
        print(f'Macro F1: {f1_score(targets, outputs, average="macro"):.4f}')

        plot_confusion_matrix(targets, outputs, all_cats, 
                      save_path=os.path.join(args.output_path, 'confusion_matrix.png'))

        print('\nClassification Report:')
        get_test_report(targets, outputs, all_cats)
